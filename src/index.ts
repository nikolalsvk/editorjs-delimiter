...
import delimiterIcon from "./icons/delimiter";
import asteriskIcon from "./icons/asterisk";
import getLineWidthIcon from "./icons/line";
import getThicknessIcon from "./icons/thickness";
...
export interface DelimiterCSS {
  block: string;
  wrapper: string;
  wrapperForStyle: (style: string) => string;
  wrapperForThickness: (thickness: number) => string;
}
...
export default class Delimiter implements BlockTool {
  private _api: API;
  private _config: DelimiterConfig;
  private _data: DelimiterData;
  private _CSS: DelimiterCSS;
  ...
  get availableDelimiterStyles() {
    if (this._config.styleOptions) {
      return Delimiter.DELIMITER_STYLES.filter((style) =>
        this._config.styleOptions?.includes(style)
      );
    }
    return Delimiter.DELIMITER_STYLES;
  }
   ...
   */
  _normalizeData(data: any): DelimiterData {
    if (typeof data !== "object") {
      data = {};
    }

    const newData: DelimiterData = {
      style: data.style || this.userDefaultDelimiterStyle,
    };
    newData.lineWidth =
  ...
  get currentLineThickness() {
    const lineThickness =
      this.availableLineThickness.find(
        (thickness) => thickness === this._data.lineThickness
      ) ?? this.userDefaultLineThickness;
  ...
  _createChildElement() {
    ...
    if (this.currentDelimiterStyle in delimiterMap) {
      ...
      child.textContent =
        delimiterMap[this.currentDelimiterStyle as keyof typeof delimiterMap];
    } else {
   ...
   */
  _setLineThickness(newThickness: number) {
   ...
   * @returns {string}
   */
  _getFormattedLabel(name: string, prefix: string, suffix?: string): string {
    if (prefix || suffix) {
  ...
  renderSettings() {
    ...
    const lineWidths = this.availableLineWidths.map((width) =>
      this._createSetting(
        ...
        this._getFormattedLabel(width.toString(), "Line ", "%"),
        () => this._setLine(width),
    ...
    let lineThickness: MenuConfigItem[] = [];
    if (this.currentDelimiterStyle === "line") {
      lineThickness = this.availableLineThickness.map((thickness) =>
        this._createSetting(
          getThicknessIcon(thickness),
          this._getFormattedLabel(thickness.toString(), "Thickness "),
    ...
    return [...settings, ...lineWidths, ...lineThickness];
  }
}
